...
class bdPhotos_Deferred_PhotoCounters extends XenForo_Deferred_Abstract
{
	public function execute(array $deferred, array $data, $targetRunTime, &$status)
	{
		...
		{
			case 'locations':
				$targetIds = $photoModel->getModelFromCache('bdPhotos_Model_Location')->getLocationIdsInRange($data['position'], $data['batch']);
			...
			case 'devices':
			default:
				$targetIds = $photoModel->getModelFromCache('bdPhotos_Model_Device')->getDeviceIdsInRange($data['position'], $data['batch']);
